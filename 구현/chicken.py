...
for store in chicken_position:
    ...
    for house in house_position:
        chicken_distance += (abs(store[0]-house[0])+abs(store[1]-house[1]))

    # 전체 거리의 합 기준 제일 작은거부터
    if len(chicken_value) == 0:
        chicken_value.append((chicken_distance, store))
    elif chicken_value[0][0] < chicken_distance:
        chicken_value.append((chicken_distance, store))
    else:
        chicken_value.insert(0, (chicken_distance, store))
    print(chicken_value)
